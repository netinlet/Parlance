 ...
 */
export const parlanceHome = (): string =>
  process.env.PARLANCE_HOME?.trim() || join(homedir(), '.parlance');
...
export const sessionFile = (root: string): string =>
  join(parlanceDir(root), '_session.json');
export const hooksDir = (root: string): string =>
  join(parlanceDir(root), 'hooks');
export const configFile = (root: string): string =>
  join(parlanceDir(root), 'config.json');
export const routingFile = (root: string): string =>
  join(parlanceDir(root), 'tool-routing.md');
export const benchStateFile = (root: string): string =>
  join(parlanceDir(root), 'bench', '_active.json');
...
export const sessionLogFile = (): string =>
  join(telemetryDir(), 'session-log.md');
export const benchResultsFile = (): string =>
  join(telemetryDir(), 'bench', 'results.jsonl');
