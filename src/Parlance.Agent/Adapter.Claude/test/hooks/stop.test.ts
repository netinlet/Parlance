...
const hook = fileURLToPath(
  new URL('../../dist/hooks/stop.js', import.meta.url),
);
...
beforeEach(() => {
  ...
  writeFileSync(
    ...
    [
      JSON.stringify({
        type: 'system',
        timestamp: '2026-04-22T14:29:00Z',
        gitBranch: 'main',
      }),
      JSON.stringify({
        type: 'assistant',
...
describe('stop hook', () => {
  it('writes a ledger entry with transcript-derived usage', () => {
    ...
    const lines = readFileSync(join(central, 'telemetry/ledger.jsonl'), 'utf8')
      .trim()
      .split('\n');
  ...
  it('appends a session log line', () => {
    ...
    const body = readFileSync(
      join(central, 'telemetry/session-log.md'),
      'utf8',
    );
