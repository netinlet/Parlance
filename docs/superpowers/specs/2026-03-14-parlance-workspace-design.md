...
`RefreshAsync` — Server mode only (throws `InvalidOperationException` in Report mode). Handles **source text changes to existing documents only**: scans loaded documents for on-disk changes, reads updated file contents, applies new `SourceText` to the Roslyn workspace via `Solution.WithDocumentText()`, then marks affected projects dirty in the compilation cache. Both the workspace's source and the cache's compiled artifacts are updated — marking caches dirty alone is not enough, because recompilation against stale `SourceText` produces stale results. Primary use case: server mode with file watching disabled (testing, debugging). With file watching enabled, this is typically unnecessary but safe to call. Increments `SnapshotVersion` if changes are detected.

**Structural changes are out of scope for Milestone 1.** Added/removed source files, `.csproj` edits, `Directory.Build.props` changes, NuGet restore/reference changes, analyzer changes, and generated-file changes are not detected by `RefreshAsync` or file watching. These require disposing the session and reopening. Structural change detection (project reload without full session rebuild) is a future enhancement.
