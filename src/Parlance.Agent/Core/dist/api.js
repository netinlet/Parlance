...
function looksLikeCsharp(root) {
  ...
  try {
    const result = readdirSync(join3(root, "src")).some(
      (e) => /\.csproj$/i.test(e)
    );
...
function parlanceMcpWired(root) {
  try {
    const config = JSON.parse(
      readFileSync4(join3(root, ".mcp.json"), "utf8")
    );
...
function codexHooksJsonReferencesSessionStart(root) {
  try {
    const config = JSON.parse(
      readFileSync4(join3(root, ".codex", "hooks.json"), "utf8")
    );
    const sessionStart = config.hooks?.SessionStart ?? [];
    return sessionStart.some(
      (entry) => entry.hooks?.some(
        (hook) => typeof hook.command === "string" && hook.command.includes(".parlance/hooks/session-start.js")
      ) ?? false
    );
  } catch {
    return false;
