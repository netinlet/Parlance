...
describe('renderForCodex', () => {
  it('SessionStart guidance becomes additionalContext', () => {
    ...
    expect(output?.hookSpecificOutput).toEqual({
      ...
      additionalContext:
        'parlance: Use Parlance first Suggested: mcp__parlance__search-symbols.',
    });
  });
...
describe('writeCodexOutput', () => {
  it('writes newline-delimited JSON', () => {
    const chunks: string[] = [];
    writeCodexOutput({ systemMessage: 'x' }, (chunk) => {
      chunks.push(chunk);
    });
