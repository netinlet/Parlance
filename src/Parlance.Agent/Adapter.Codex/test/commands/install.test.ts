...
describe('install command', () => {
  ...
  it('preserves foreign hooks without command fields', async () => {
    ...
    expect(
      pre.some((entry) =>
        entry.hooks.some(
          (hook) =>
            hook.type === 'webhook' && hook.url === 'https://example.test/hook',
        ),
      ),
    ).toBe(true);
  });
  ...
  it('writes Codex MCP setup guidance using the solution path', async () => {
    ...
    const body = readFileSync(
      join(root, '.parlance/codex/mcp-setup.md'),
      'utf8',
    );
    expect(body).toContain(
      'codex mcp add parlance -- parlance mcp --solution-path',
    );
  ...
  it('uses --mcp-command in Codex MCP setup guidance', async () => {
    ...
    expect(body).toContain(
      'codex mcp add parlance -- /opt/parlance/bin/parlance mcp --solution-path',
    );
  });
...
describe('install --global', () => {
  const orig = {
    ...
    codex: process.env.CODEX_CONFIG_DIR,
  };

  afterEach(() => {
    for (const [key, value] of [
      ['PARLANCE_HOME', orig.home],
      ['CODEX_CONFIG_DIR', orig.codex],
    ] as const) {
  ...
  it('copies the nudge bundle and wires a nudge-only SessionStart hook', async () => {
    ...
    const sessionStart = parsed.hooks.SessionStart as {
      hooks: { command: string; statusMessage?: string }[];
    }[];
    expect(
    ...
    expect(
      sessionStart.some((entry) =>
        entry.hooks.some(
          (hook) => hook.statusMessage === 'Checking Parlance setup',
        ),
      ),
    ).toBe(true);
